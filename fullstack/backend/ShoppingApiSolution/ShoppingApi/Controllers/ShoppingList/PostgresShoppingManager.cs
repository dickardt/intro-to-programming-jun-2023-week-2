...
public class PostgresShoppingManager : IManageTheShoppingList
{
    ...
    public async Task<ShoppingListItemModel> AddItemAsync(ShoppingListItemModel model)
    {
    ...
    {
        var results = await _context.ShoppingList.Select(item => new ShoppingListItemModel
        {
            Id = item.Id.ToString(),
    ...
    {
        var saveItem = await _context.ShoppingList.SingleOrDefaultAsync(item => item.Id == int.Parse(request.Id));
        if (saveItem != null)
        {
            saveItem.Purchased = true;
            await _context.SaveChangesAsync();
            return true;
        }
        else
